...
export default {
  ...
  methods: {
    ...
    calculateInitialPayment() {
      let total = this.plans[this.selectedPlan].price + 3300
      if (this.safetyOption) total += 660
      return total
    },
    calculateRecurringPayment() {
      let total = this.plans[this.selectedPlan].price
      if (this.safetyOption) total += 660
      return total
    },
  },
...
<template>
  <v-app style="height: 100vh;">
    <v-app-bar title="title">
      <nuxt-link to="/MyPage/mypage">マイページ</nuxt-link>
    </v-app-bar>
